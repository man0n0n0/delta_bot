...
class RockStacking(Node):
    ...
    def stage2_callback(self, msg):
        ...
        self.send_gcode('G91')
        self.send_gcode(f'G1 X{(-1*self.rock_x)+self.tool_offset[0]:.1f} Y{self.rock_y+self.tool_offset[1]:.1f} F2000') #correction with negative value because of the relative movement

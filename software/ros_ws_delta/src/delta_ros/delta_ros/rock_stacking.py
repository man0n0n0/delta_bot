...
class RockStacking(Node):
    ...
    def stage1_callback(self, msg):
        ...
        rock = msg.poses[int(len(msg.poses)*0.75)-1]  # go toward the smallest
        self.rock_x = rock.position.x * 1000
        self.rock_y = rock.position.y * 1000
        self.rock_z = rock.position.z * 1000

        self.rock_first_approch = self.rock_z * 0.35
    ...
    def stage3_callback(self, msg):
        ...
        self.send_gcode(f'G1 X{self.rock_x*self.approch_coeff+self.tool_offset[0]:.1f} Y{(self.rock_y*self.approch_coeff)-self.tool_offset[1]:.1f} F{self.unloaded_speed} ') #correctected offset
        ...
        time.sleep(10)
        self.send_gcode('G91')
        self.send_gcode(f'G1 Z{pick_plunge + self.rock_first_approch - self.safe_height:.1f} F500')  # Lift rock
        ...
        if self.saved_plane_distance is not None:
            ...
            drop_plunge = (self.placing_z - self.safe_height - (self.saved_plane_distance - self.safe_height - self.rock_z) - self.tool_offset[2]) #in mm

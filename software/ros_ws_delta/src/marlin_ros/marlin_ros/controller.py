...
# Using a standard action type from example_interfaces
...
class MarlinController(Node):
    ...
    def __init__(self):
        ...
        # Regular publisher/subscriber approach for sending G-code
        self.gcode_sub = self.create_subscription(
            String,
            f'/{self.machine_id}/gcode',
            self.gcode_callback,
            10
        )
    ...
    def gcode_callback(self, msg):
        """Callback for receiving G-code commands via topic"""
        cmd = msg.data
        self.get_logger().info(f"Received G-code command: {cmd}")
